		...
		function attachFormValidator(elements) {
			...
			regula.custom({
				...
				validator: function () {
					...
					else return /^\+?[0-9\-\(\) ]{10,11}$/i.test(this.value);
				}
			});
			...
			for (var i = 0; i < plugins.swiper.length; i++) {
				...
				for (var j = 0; j < swiperSlide.length; j++) {
					...
					if (url = $this.attr("data-slide-bg")) {
						$this.css({
							"background-image": "linear-gradient(#000000, #00000055), url(" + url + ")",
				...
				$campaignItem.on('submit', $.proxy(function (e) {
					...
					$.ajax({
						...
						error: function (resp, text) {
							...
							$output.html('Erro: ' + (responseJSON && responseJSON.message || "Falha ao processar no servidor"));
